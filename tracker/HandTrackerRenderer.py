...
class HandTrackerRenderer:
    ...
    def waitKey(self, delay=1, mean_depth=0, person_infront=False, action=""):
        cv2.putText(self.frame, f"Mean Depth: {mean_depth.round(2)}", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        cv2.putText(self.frame, f"Person in front: {person_infront}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        cv2.putText(self.frame, f"Action: {action}", (10, 130), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        if self.show_fps:
                self.tracker.fps.draw(self.frame, orig=(self.frame.shape[1]-180,50), size=1, color=(0,255,0))
